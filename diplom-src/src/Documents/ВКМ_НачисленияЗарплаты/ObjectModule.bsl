...
Процедура РасчитатьОклад(); 
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТ_РасчетЗППоОкладу
		|ИЗ
		|	ВТ_Отработано КАК ВТ_Отработано
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ДатыУстановкиОклада КАК ВТ_ДатыУстановкиОклада
		|		ПО ВТ_Отработано.НомерСтроки = ВТ_ДатыУстановкиОклада.НомерСтроки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВКМ_УсловияОплатыСотрудников КАК ВКМ_УсловияОплатыСотрудников
		|		ПО ВТ_Отработано.Сотрудник = ВКМ_УсловияОплатыСотрудников.Сотрудник
		|			И (ВТ_ДатыУстановкиОклада.Период = ВКМ_УсловияОплатыСотрудников.Период)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_РасчетЗППоОкладу.НомерСтроки КАК НомерСтроки,
		|	ВТ_РасчетЗППоОкладу.Сотрудник КАК Сотрудник,
		|	ВТ_РасчетЗППоОкладу.План КАК План,
		|	ВТ_РасчетЗППоОкладу.Факт КАК Факт,
		|	ВТ_РасчетЗППоОкладу.Показатель КАК Показатель,
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботы.СуммаКОплате, 0)) КАК СуммаКОплате
		|ИЗ
		|	ВТ_РасчетЗППоОкладу КАК ВТ_РасчетЗППоОкладу
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВКМ_ВыполненныеСотрудникомРаботы КАК ВКМ_ВыполненныеСотрудникомРаботы
		|		ПО ВТ_РасчетЗППоОкладу.Сотрудник = ВКМ_ВыполненныеСотрудникомРаботы.Сотрудник
		|			И (ВКМ_ВыполненныеСотрудникомРаботы.Период МЕЖДУ НАЧАЛОПЕРИОДА(ВТ_РасчетЗППоОкладу.ПериодДействия, МЕСЯЦ) И КОНЕЦПЕРИОДА(ВТ_РасчетЗППоОкладу.ПериодДействия, МЕСЯЦ))
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_РасчетЗППоОкладу.Сотрудник,
		|	ВТ_РасчетЗППоОкладу.План,
		|	ВТ_РасчетЗППоОкладу.Факт,
		|	ВТ_РасчетЗППоОкладу.Показатель,
		|	ВТ_РасчетЗППоОкладу.НомерСтроки";  
